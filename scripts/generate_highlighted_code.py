...
files = []
try:
	files = os.listdir("resources/highlight_code")
	print("-- Listing files in 'resources/highlight_code' directory")
except:
	print("-- FATAL: Directory 'resources/highlight_code' does not exist. Make sure that the script is run from the root directory of the project, or inside 'script'.")
	input("Press enter to continue...")
	exit()

files = [f for f in files if f.endswith(".ct")]
if len(files) == 0:
	print("-- FATAL: No files ending with '.ct' found in 'resources/highlight_code' directory, aborting...")
	input("Press enter to continue...")
	exit()
else:
...
for filename in files:
	content = open("resources/highlight_code/{}".format(filename), "r").read()
